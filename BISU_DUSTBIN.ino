...
const int virtualPins[] = {V0, V1, V2}; // Blynk virtual pins
  ...
  if (distances[sensorIndex] > 0 && distances[sensorIndex] <= 10) {
    if (!ledStates[sensorIndex]) {
      if (timers[sensorIndex] == 0) {
        ...
        String notificationMessage = (sensorIndex == 0) ? "Bin-BIO -- @BISU is FULL" :
                                     (sensorIndex == 1) ? "Bin-NONBIO -- @BISU is FULL" :
                                                          "Bin-Hazard -- @BISU is FULL";
